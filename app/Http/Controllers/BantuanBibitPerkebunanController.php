...
use App\Models\BantuanBibitPerkebunanDetail;
use Illuminate\Http\Request;
use Yajra\DataTables\Facades\DataTables;

class BantuanBibitPerkebunanController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = BantuanBibitPerkebunan::with(['kelompokTani', 'komoditas'])->select('bantuan_bibit_perkebunans.*');
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $editUrl = route('bantuan-bibit-perkebunan.edit', $row->id);
                    $deleteUrl = route('bantuan-bibit-perkebunan.destroy', $row->id);
                    return '
        ...
        return view('produksi.perkebunan.bantuan.index');
    }
    ...
    public function create()
    {
        ...
        $komoditas = Komoditas::whereHas('kategori', function($q) {
            $q->where('nama', 'like', '%Perkebunan%');
        })->pluck('nama', 'id')->toArray();

        return view('produksi.perkebunan.bantuan.create', compact('kelompokTanis', 'komoditas'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $bantuan = BantuanBibitPerkebunan::create($validated);

        foreach ($detailsData as $detail) {
            $bantuan->details()->create($detail);
        }

        return redirect()->route('bantuan-bibit-perkebunan.index')
            ->with('success', 'Data Bantuan Bibit Perkebunan berhasil ditambahkan.');
    }

    public function edit($id)
    {
        $bantuan = BantuanBibitPerkebunan::findOrFail($id);
        $kelompokTanis = KelompokTani::pluck('nama', 'id')->toArray();
        $komoditas = Komoditas::whereHas('kategori', function($q) {
            $q->where('nama', 'like', '%Perkebunan%');
        })->pluck('nama', 'id')->toArray();
        ...
        return view('produksi.perkebunan.bantuan.edit', compact('bantuan', 'kelompokTanis', 'komoditas', 'petanis', 'existingDetails'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->route('bantuan-bibit-perkebunan.index')
            ->with('success', 'Data Bantuan Bibit Perkebunan berhasil diperbarui.');
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('bantuan-bibit-perkebunan.index')
            ->with('success', 'Data Bantuan Bibit Perkebunan berhasil dihapus.');
    }
}
