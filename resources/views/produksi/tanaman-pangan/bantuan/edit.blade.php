...
@section('title', 'Edit Bantuan Benih Tanaman Pangan')
...
<x-breadcrumb :items="[
    ...
    ['label' => 'Bantuan Benih', 'url' => route('bantuan-benih-pangan.index')],
    ['label' => 'Edit Bantuan']
]" />

<form action="{{ route('bantuan-benih-pangan.update', $bantuan->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-md-6">
            <div class="card custom-card border-0 p-4 mb-4">
                <h5 class="fw-bold mb-4">Form Edit Bantuan Benih</h5>
                ...
                <div class="mb-3">
                    ...
                    <select id="kelompok_tani_id" name="kelompok_tani_id" class="form-select rounded-3 border-secondary-subtle" required>
                        <option value="{{ $bantuan->kelompok_tani_id }}" selected>
                            {{ $bantuan->kelompokTani->nama }} - {{ $bantuan->kelompokTani->desa ? $bantuan->kelompokTani->desa->nama : '-' }} - {{ $bantuan->kelompokTani->ketua ?: '-' }}
                        </option>
                    </select>
                </div>
                ...
                <x-form.select 
                    name="varietas_id" 
                    label="Varietas" 
                    placeholder="-- Pilih Varietas (Opsional) --" 
                    :options="$varietas"
                    selected="{{ $bantuan->varietas_id }}"
                />
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <x-form.input 
                            name="satuan" 
                            label="Satuan" 
                            placeholder="Contoh: Kg, Ton" 
                            required="true" 
                            value="{{ $bantuan->satuan }}"
                        />
                    </div>
                </div>
                <div class="row">
                ...
                <div class="d-flex justify-content-between mt-4">
                    <a href="{{ route('bantuan-benih-pangan.index') }}" class="btn btn-light rounded-3 px-4">Batal</a>
                    <button type="submit" class="btn btn-success rounded-3 px-4">Perbarui Data</button>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="card custom-card border-0 p-4">
                ...
                <p class="text-muted small mb-4">Ubah jumlah bantuan yang didapatkan oleh masing-masing anggota di bawah ini.</p>
                            ...
                            @if(count($petanis) > 0)
                                @foreach($petanis as $petani)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" step="0.01" min="0" name="petani_jumlah[{{ $petani->id }}]" class="form-control form-control-sm petani-jumlah-input rounded-3 border-secondary-subtle" value="{{ $existingDetails[$petani->id] ?? '' }}" placeholder="0">
                                        </td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
                                    <td colspan="3" class="text-center text-muted">Kelompok tani ini belum memiliki anggota petani</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
