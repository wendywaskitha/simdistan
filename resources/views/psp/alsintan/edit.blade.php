...
@section('title', 'Edit Penerima Bantuan Alsintan')
...
<div class="row">
    <div class="col-lg-10 col-xl-8">
        <div class="card custom-card border-0 p-4">
            <h5 class="fw-bold mb-4">Form Edit Penerima Bantuan Alsintan</h5>

            <form action="{{ route('alsintans.update', $alsintan->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select id="kelompok_tani_id" name="kelompok_tani_id" required class="form-select @error('kelompok_tani_id') is-invalid @enderror">
                                    <option value="" disabled {{ is_null(old('kelompok_tani_id', $alsintan->kelompok_tani_id)) ? 'selected' : '' }}>Pilih Kelompok Tani</option>
                                    @foreach($kelompokTanis as $item)
                                        <option value="{{ $item->id }}" data-ketua="{{ $item->ketua }}" {{ old('kelompok_tani_id', $alsintan->kelompok_tani_id) == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select id="jenis_alat_id" name="jenis_alat_id" required class="form-select @error('jenis_alat_id') is-invalid @enderror">
                                    <option value="" disabled {{ is_null(old('jenis_alat_id', $alsintan->jenis_alat_id)) ? 'selected' : '' }}>Pilih Jenis Alat</option>
                                    @foreach($jenisAlats as $jenis)
                                        <option value="{{ $jenis->id }}" {{ old('jenis_alat_id', $alsintan->jenis_alat_id) == $jenis->id ? 'selected' : '' }}>
                                            {{ $jenis->nama }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <x-form.select 
                                ...
                                label="Tahun Bantuan" 
                                :options="$years" 
                                selected="{{ $alsintan->tahun_bantuan }}"
                                required="true"
                            />
                        </div>
                    </div>
